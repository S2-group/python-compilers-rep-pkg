...
def read_kn_inputs():
    files = glob.glob('./functions/knucleotide_inputs/*.txt')
    inputs = []
    for x in files:
...
def check_output_subprocess(inputs, fun_to_test, fun_horacle, inputs_horacle=""): 
    ...
    for x, y in zip(inputs, inputs_horacle):
        output =  sb.run(
            [
                f"./codon/codon_tests/build/{fun_to_test}", str(x)
            ], capture_output=True
        ).stdout.decode('utf-8')
        expected_output = capture_stdout(fun_horacle, y) 

        if fun_to_test == 'n_body' or fun_to_test == 'spectralnorm':
...
def test_numba():
    ...
    check_output(cases["n_body"], nbnb.main, pynb.main)
...
def test_cython():
    ...
    check_output(cases["n_body"], cynb.main, pynb.main)
    check_output(cases["spectralnorm"], cysp.main, pysp.main)

def test_mypyc():
    import sys
    sys.path.append('./mypyc/build')
    ...
    check_output(cases["n_body"], mynb.main, pynb.main)
...
def test_codon():
    ...
    check_output_subprocess(
        glob.glob('./functions/knucleotide_inputs/*.txt'),
        "k_nucleotide",
        pykn.main,
        inputs_horacle=cases["k_nucleotide"]
    )
    check_output_subprocess(cases["mandelbrot"], "mandelbrot", pyma.main)
    check_output_subprocess(cases["n_body"], "n_body", pynb.main)
